...
export default class UserClass extends sql_class {
  ...
  selectFromDiscordId(data: number): Promise<any> {
    const name = this.tableName;
    return new Promise((resolve, reject) => {
      database.query(
        `SELECT * FROM ${name} WHERE DISCORD_ID=?`,
        [data],
        function (err, results) {
          if (err) reject(err);
          if (results == undefined) resolve(undefined);
  ...
  delFromDiscordId(data: number): Promise<void> {
    const name = this.tableName;
    ...
    return new Promise(async (resolve, reject) => {
      ...
      database.query(
        `DELETE FROM ${name} WHERE DISCORD_ID=?`,
        [data],
        function (err, results) {
          if (err) reject(err);
          console.log(
